...
export const useGame = () => {
  ...
  const gameComponentValue = useComponentValue(
    Game,
    useEntityQuery([HasValue(Game, { id: game_id })])
  );
  ...
  const current_turn = sanitizedGame
    ? Math.floor(sanitizedGame.nonce / (3 * sanitizedGame.player_count) + 1)
    : 0;
  const number_max_turns = sanitizedGame
    ? Math.floor(sanitizedGame.limit / (3 * sanitizedGame.player_count))
    : 0;
