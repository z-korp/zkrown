...
export const useLogs = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async (gameId: number) => {
      ...
      await fetchEventsOnce(
        [SUPPLY_EVENT, "0x" + gameId.toString(16)],
        async (event: Event) => {
          addLogIfUnique(generateLogFromEvent(event));
        }
      );
      await fetchEventsOnce(
        [FORTIFY_EVENT, "0x" + gameId.toString(16)],
        async (event) => addLogIfUnique(generateLogFromEvent(event))
      );
      await fetchEventsOnce(
        [DEFEND_EVENT, "0x" + gameId.toString(16)],
        async (event) => {
  ...
  const fetchBattleEvents = async (gameId: number, transactionHash: string) => {
    ...
    await fetchEventsOnce(
      [BATTLE_EVENT, "0x" + gameId.toString(16), transactionHash],
      async (event) => {
        const battleEvent = parseBattleEvent(event);
        battleEvents.push(battleEvent);
      }
    ...
    if (game_id !== undefined && game?.seed !== 0 && playerNames.length !== 0) {
      ...
      if (!subscribedRef.current) {
        ...
        const subscribeToEvents = async () => {
          const supplyObservable = await createSupplyEvents(
            "0x" + game_id.toString(16)
          );
          const defendObservable = await createDefendEvents(
            "0x" + game_id.toString(16)
          );
          const fortifyObservable = await createFortifyEvents(
            "0x" + game_id.toString(16)
          );
            ...
            defendObservable
              .pipe(
                mergeMap(async (event) => {
                  ...
                  const log = generateLogFromEvent(event);
                  const battleEvents = await fetchBattleEvents(
                    game_id,
