...
const BattleRound: React.FC<BattleRoundProps> = ({ battle, round }) => {
  ...
  const attackerDices = battle.rounds[round]
    .map((duel) => duel.attackerValue)
    .filter((value) => value !== 0);
  const defenderDices = battle.rounds[round]
    .map((duel) => duel.defenderValue)
    .filter((value) => value !== 0);

  const dicesBattle = Math.min(attackerDices.length, defenderDices.length);

  const attackerWon = attackerDices
    .slice(0, dicesBattle)
    .map((value, index) => {
      ...
      <div className="flex flex-row h-[288px]">
        <div className="flex flex-col gap-2">
          {attackerDices.slice(0, dicesBattle).map((d, index) => (
            <div
              ...
              key={`attacker-${round}-${index}`}
            >
              {!attackerWon[index] && (
                <span className="text-red-500 w-6">{`${
                  showPenalty ? "-1" : ""
                }`}</span>
              )}
          ...
          {attackerDices.slice(dicesBattle).map((d, index) => (
            <div
              ...
              key={`attacker-${round}-${index + dicesBattle}`}
            >
        ...
        <div className="flex flex-col gap-2">
          ...
          {defenderDices.slice(dicesBattle).map((d, index) => (
            <div
              className="flex items-center"
              key={`defender-${round}-${index + dicesBattle}`}
            >
              <Dice scale={0.3} desiredResult={d} />
              <span className="w-6"></span>
            </div>
