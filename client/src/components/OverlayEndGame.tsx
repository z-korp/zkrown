        ...
        >
          <div className="relative flex flex-col items-center w-1/2 rounded-lg bg-stone-700 border-2 border-stone-900 pb-16 pt-16">
            ...
            <div className="flex flex-col items-center justify-center w-full">
              {[...players]
                .sort(
                  (playerA: any, playerB: any) => playerA.rank - playerB.rank
                )
                .map((player: any, index: number) => (
                  <div
                    className="flex gap-4 mb-2 items-center justify-center text-white text-lg w-full"
                    key={index}
                  >
                    {player.rank !== 0 && (
                      <>
                        {player.rank === 1 && (
                          <Trophy className="w-8 h-8" stroke="gold" />
                        )}
                        {player.rank === 2 && (
                          <Medal className="w-8 h-8" stroke="silver" />
                        )}
                        {player.rank === 3 && (
                          <Medal
                            className="w-8 h-8"
                            stroke={getColorRGB("bronze")}
                          />
                        )}
                        {player.rank !== 1 &&
                          player.rank !== 2 &&
