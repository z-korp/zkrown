...
import {
  colorTilePlayer,
  colorTilePlayerHighlight,
} from "@/utils/customColors";
import { getAllConnectedTiles, getNeighbors } from "@/utils/map";
import {
  calculateCentroidFromPath,
  transformPointFromSVGToScreen,
} from "@/utils/svg";
...
const Region: React.FC<RegionProps> = ({ id, containerRef, onRegionClick }) => {
  ...
  const colorTileHighLight = tile
    ? colorTilePlayerHighlight[tile.owner + 1 || 0]
    : "white";
  ...
  useEffect(() => {
    ...
    if (path && pathData) {
      ...
      if (svgElement) {
        const pointInScreenFrame = transformPointFromSVGToScreen(
          svgElement,
          centroid
        );
