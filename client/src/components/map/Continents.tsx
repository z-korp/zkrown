...
const Continents: React.FC<ContinentsProps> = ({ containerRef }) => {
  ...
  const [highlightedContinents, setHighlightedContinents] = useState<
    Set<number>
  >(new Set());
  ...
  useEffect(() => {
    ...
    continentsData.continents.forEach((continent) => {
      const playerIndexWhoOwnsContinent = players.find((player) =>
        continent.regions.every((regionId) =>
          tiles.find(
            (tile) => tile.id === regionId && tile.owner === player.index
          )
        )
      )?.index;
    ...
    <>
      {data.continents.map((continent) => (
        <Continent
          ...
          continent={continent}
          isVisible={
            isContinentMode ? true : highlightedContinents.has(continent.id)
          }
