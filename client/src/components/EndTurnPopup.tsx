...
const EndTurnPopup: React.FC<EndTurnPopupProps> = ({ cards, onClose }) => {
  ...
  useEffect(() => {
    const playerTilesSet = new Set(
      tiles.filter((t) => t.owner === (me?.index ?? null)).map((t) => t.id)
    );
    const conqueredThisTurnSet = new Set(tilesConqueredThisTurn);

    const continentsToHighlight = continentsData.continents
      .filter((continent) =>
        continent.regions.every((region) => playerTilesSet.has(region))
      )
      .filter((continent) =>
        continent.regions.some((region) => conqueredThisTurnSet.has(region))
      )
    ...
    <div className="absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center z-50 font-vt323 text-2xl text-white">
      <div className="relative bg-stone-700 border-2 border-stone-900 p-6 rounded-lg shadow-lg text-center pointer-events-auto">
        ...
        <div className="max-w-md text-white mb-4">
          ...
          <p className="text-lg">
            Your successful conquests have earned you this card. Use it wisely
            to fortify your dominion.
          </p>
        </div>
        ...
        {continentNames.length > 0 && (
          <div className="text-lg mt-4">
            <p>
              You have conquered the following continent
              {continentNames.length > 1 ? "s" : ""}:
            </p>
